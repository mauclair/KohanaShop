<?php
   class Basket_Model extends Model{
		...
		public function addItem($lid,$count=1){						
			if ($count <=0) return;			
			$expiration = time() + Kohana::config('main.ticket_expiration')*60;
		...
		public function modify(&$a){			
			...
			if (!$data->validate()) { error::parseValidation($data->errors()); return false;}
			$data = $data->as_array();
			$vid = $data['product_id'];
			$pocet = $data['quantity'];
			...
			if ($pocet <= 0 ){ // odebira se na nulu, nebo tak
				$this->remove($vid);
			}else if ($pocet>$item['count'] ){ // zkusime pridat, pokud che vic
