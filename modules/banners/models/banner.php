  ...
  //  public $validation = array('banner_file'=>'required');
    ...
    public function getBanners(){
        ...
        foreach($bcdata as $cat) {
            ...
            $res[$hid] = array();
            $bm = new Banner_Model();
            $bm->where->and('banner_group', $hid);
            $bm->where->and(condition::factory()->or('display_from',date('Y-m-d'),'<=')->or('display_from','0000-00-00'));
            $bm->where->and(condition::factory()->or('display_to',date('Y-m-d'),'>=')->or('display_to','0000-00-00'));
            $bm->where->and(condition::factory()->or('display_clicks',0)->or('display_clicks', 'clicked','>=',false ));
